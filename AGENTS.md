...
npm run snapshot # SHEET_ID=... refresh src/data/snapshot.json from the Sheet
```
...
| --------------------------------------- | ------------------------------------------------------------- |
| `src/lib/{csv,schema,sheet,select}.ts`  | architecture.md · #sheet-contract #resilience #server-only    |
...
file this repo does not control. The three status _values_ are mapped to
English in exactly one place, `statusLabel` in
`src/components/status-badge.tsx`; read it before adding a status anywhere.

**A bad row never blanks a page.** Rows validate one at a time; a failure is
skipped and reported at `/purge`, never thrown. Three layers, all must hold:
skip-row → `snapshot.json` fallback → CDN stale-while-revalidate.

**`src/lib/sheet.ts` is server-only.** It carries the fetch and the snapshot
import. Components import selectors from `src/lib/select.ts` instead. Check
after changing either: `grep -l "Pogung\|Batam Centre" dist/client/assets/*.js`
must find nothing.

**Every catalogue route sends `sheetCacheHeaders`** from `src/lib/http.ts`. A
route without them silently drops to no CDN caching.
...
│   ├── sheet.ts     SERVER ONLY: fetch, 5-min cache, snapshot fallback
│   ├── select.ts    pure selectors, safe to import anywhere
│   ├── catalog.ts   server fns (loadCatalog, purgeSheetCache)
│   └── http.ts      sheetCacheHeaders
├── data/snapshot.json   committed fallback; regenerate, never hand-edit
...
- Build runs without `SHEET_ID` on purpose and serves the snapshot. Keep it that way.
- `/purge` needs `PURGE_SECRET`; without it the page says so rather than 500ing.
- Dark mode is `prefers-color-scheme` only — no class, no JS, no toggle.
...
- UI copy is English; `docs/` client handover stays Indonesian.
